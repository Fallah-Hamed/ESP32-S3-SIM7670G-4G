...
public:
        ...
        uint32_t keepalive_time = 60)
    {
        ...
        const char* scheme = __ssl ? "ssl://" : "tcp://";
        if (username && password) {
            thisModem().sendAT("+CMQTTCONNECT=", clientIndex, ',', '"', scheme, server, ':', port, "\",", keepalive_time, ',', 1, ",\"", username, "\",\"", password, "\"");
        } else {
            thisModem().sendAT("+CMQTTCONNECT=", clientIndex, ',', '"', scheme, server, ':', port, "\",", keepalive_time, ',', 1);
        }
